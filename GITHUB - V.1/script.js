    ...
    try {
        ...
        try {
            ...
            userRepos.map((repo)=>{
                ...
                /**
                 * ternary operator ?: - here used to check if the property in the json value is present make use 
                 * of that value, if not present leave/have alternate text to that field, this is because sometimes 
                 * in json we might see missing property for some objects, they can lead to error in some cases while 
                 * iterating to avoid that we can use this, this can also be done using "optional chaining" method ?. , 
                 * only if it is present display it to the user otherwise don't
                 */
